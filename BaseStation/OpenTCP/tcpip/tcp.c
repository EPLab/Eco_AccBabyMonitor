...
INT16 process_tcp_out (INT8 sockethandle, UINT8* buf, UINT16 blen, UINT16 dlen)
{
	...
	*buf++ = 0;
	...
	cs = ip_checksum(cs, (dlen + MIN_TCP_HLEN) >> 8, cs_cnt++);
	cs = ip_checksum(cs, dlen + MIN_TCP_HLEN, cs_cnt++);

	/* Go to TCP header + data	*/
	
	buf = buf_start;
	
	cs = (UINT16)(ip_checksum_buf(cs, buf, dlen + MIN_TCP_HLEN));
	...
	*buf = (UINT8)cs;
	...
	(void)process_ip_out(soc->rem_ip, IP_TCP, soc->tos, 100, buf_start, dlen + MIN_TCP_HLEN);
...
void tcp_sendcontrol (INT8 sockethandle)
{
	...
	(void)process_tcp_out(sockethandle, &tcp_tempbuf[0], MIN_TCP_HLEN + 1, 0);
...
void tcp_newstate (struct tcb* soc, UINT8 nstate)
{
	...
	switch(soc->state) {
		...
		case TCP_STATE_SYN_SENT:	
			...
			init_timer((UINT8)(soc->retransmit_timerh), TCP_INIT_RETRY_TOUT*TIMERTIC);
	...
	/* KeepAlive timer	*/
	if(soc->state == TCP_STATE_CONNECTED)
		init_timer((UINT8)(soc->persist_timerh), soc->tout);
...
UINT8 tcp_check_cs (struct ip_frame* ipframe, UINT16 len)
{
	...
	{		
		...
		cs = (UINT16)(ip_checksum_buf(cs, tcp_tempbuf,16));
		len -= 16;	
		cs_cnt += 16;
	}
