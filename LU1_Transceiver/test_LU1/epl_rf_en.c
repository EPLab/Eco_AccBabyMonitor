...
void epl_rf_en_init(unsigned char in_channel, 		// Freq = in_channel + 2400 MHz
			 ...
			 uint8_t in_auto_retr,				//
			 uint16_t in_auto_retr_delay,		//
			 char in_addr_width, 			// Address width for both PTX and PRX
			 epl_rf_en_crc_mode_t in_crc_mode,	//
			 epl_rf_en_spi_clk_rate_t in_spi_clk_rate,	//
			 bool in_rf_int)					//
{
	//--- RF SPI Configurations ---//
	RFCTL = 0x010 + in_spi_clk_rate; // Enable RF SPI and set the clock rate of SPI.
						...
						epl_rf_en_auto_ack_t in_auto_ack)					// Auto ack enable bit.
{
	...
	hal_nrf_set_rx_pload_width(in_pipe_num, in_pipe_pload_length);
}
 ...
 */
void epl_rf_en_easy_start_tx(unsigned char channel, uint8_t auto_retr, uint16_t delay){
 ...
 */
void epl_rf_en_easy_start_rx(unsigned char channel, unsigned char pload_len, bool auto_ack){
 ...
 */
void epl_rf_en_set_dst_addr(uint8_t *in_dst_addr)
{
 ...
 */
void epl_rf_en_send_dst(uint8_t *in_dst_addr, uint8_t *in_tx_pload, uint8_t in_pload_length)
{
	hal_nrf_set_address(HAL_NRF_TX, in_dst_addr); // Address for PTX (The address of destination.)
	epl_rf_en_enter_tx_mode();
	hal_nrf_write_tx_pload(in_tx_pload, in_pload_length);
	CE_PULSE();
}
 ...
 * @param in_length The packet length.
 */
void epl_rf_en_send(uint8_t *in_tx_pload, uint8_t in_pload_length)
{
	epl_rf_en_enter_tx_mode();
	hal_nrf_write_tx_pload(in_tx_pload, in_pload_length);
	CE_PULSE();
 ...
 */
bool epl_rf_en_rx_pload_is_empty(void)
{
...
{
	hal_nrf_read_rx_pload(out_rx_pload);
